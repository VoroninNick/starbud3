class DPlinthCollection < ActiveRecord::Base
  ...
  belongs_to :d_plinth_brand
  attr_accessible :d_plinth_brand_id
  ...
  def self.find_names_by_brand_self(name)
    query = "select c.name as name from d_plinth_brands b, d_plinth_collections c where b.id = c.d_plinth_brand_id and b.name = '#{name}'"
  ...
  rails_admin do
    parent DPlinthBrand
    label 'Плинтус Колекція'
    label_plural 'Плінтус Колекція'

    list do
      field :name do
        label 'Назва  колекції'
      end
      field :d_plinth_brand do
        label 'Бренд'
      end
    end
